...
require_once(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/braintree/Braintree.php');
...
class braintree_api extends base {
	...
	var $_logDir = 'includes/modules/payment/braintree/logs/';
	var $_logLevel = 0;
	...
	function braintree_api() {
		...
		if(IS_ADMIN_FLAG === true) {
			...
			if($this->enabled) {
				...
				if(MODULE_PAYMENT_BRAINTREE_SERVER =='sandbox') $this->title .= '<strong><span class="alert"> (sandbox active)</span></strong>';
				if(MODULE_PAYMENT_BRAINTREE_DEBUGGING =='Log File' || MODULE_PAYMENT_BRAINTREE_DEBUGGING =='Log and Email') $this->title .= '<strong> (Debug)</strong>';
		...
		// DEV MODE:
		if(defined('BRAINTREE_DEV_MODE') && BRAINTREE_DEV_MODE == 'true') $this->_logLevel = 3;
	...
	*/
	...
	function selection() {
		...
		$this->cc_type_check =
		'var value = document.checkout_payment.braintree_cc_type.value;' .
		...
		*/
		...
		if (sizeof($this->cards)>0) $fieldsArray[] = array('title' => MODULE_PAYMENT_BRAINTREE_TEXT_CREDIT_CARD_TYPE,
	...
	*/
	...
	function pre_confirmation_check() {
		...
		$result = $cc_validation->validate($_POST['braintree_cc_number'],
					$_POST['braintree_cc_expires_month'], $_POST['braintree_cc_expires_year'],
					(isset($_POST['braintree_cc_issue_month']) ? $_POST['braintree_cc_issue_month'] : ''), (isset($_POST['braintree_cc_issue_year']) ? $_POST['braintree_cc_issue_year'] : ''));
		$error = '';
		...
		if(isset($_POST['braintree_cc_issuenumber'])) $_POST['braintree_cc_issuenumber'] = preg_replace('/[^0-9]/i', '', $_POST['braintree_cc_issuenumber']);
	...
	*/
	...
	function after_process() {
		...
		zen_db_perform('braintree', $braintree_order);
	...
	*/
	...
	function admin_notification($zf_order_id) {
		...
		if(!defined('MODULE_PAYMENT_BRAINTREE_STATUS')) return '';
		global $db;
		...
		$braintree_order_details = $db->Execute($sql);
		if($braintree_order_details->RecordCount() == 0) $braintree_order_details->fields = array();

		if(file_exists(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/braintree/braintree_admin_notification.php')) include_once(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/braintree/braintree_admin_notification.php');
	...
	*/
	...
	function _GetTransactionDetails($oID) {
		...
		*/
		...
		$response['SHIPTOSTREET'] = $result->shippingDetails->streetAddress;
		$response['SHIPTOSTREET2'] = $result->shippingDetails->extendedAddress;
		$response['SHIPTOCITY'] = $result->shippingDetails->locality;
		$response['SHIPTOSTATE'] = $result->shippingDetails->region;
		$response['SHIPTOZIP'] = $result->shippingDetails->postalCode;
		$response['SHIPTOCOUNTRY'] = $result->shippingDetails->countryName;
	...
	*/
	...
	function install() {
		...
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Version', 'MODULE_PAYMENT_BRAINTREE_VERSION', '1.1.0', 'Version installed (do not change this value unless you would like the automatic upgrade to run)', '6', '25', now())");
	...
	*/
	...
	function zcLog($stage, $message) {
		...
		if(MODULE_PAYMENT_BRAINTREE_DEBUGGING == 'Log and Email' || MODULE_PAYMENT_BRAINTREE_DEBUGGING == 'Log File') {
			...
			$token .= $tokenHash;
			$file = $this->_logDir . '/' . $this->code . '_Braintree_Action_' . $token . '.log';
			if(defined('BRAINTREE_DEV_MODE') && BRAINTREE_DEV_MODE == 'true') $file = $this->_logDir . '/' . $this->code . '_Braintree_Debug_' . $token . '.log';
			$fp = @fopen($file, 'a');
		...
		*/
		...
		if($proceedToRefund) {
			...
			if($result->success) {
				...
				if(!isset($result->transaction->amount)) $result->transaction->amount = $refundAmt;
				...
				$messageStack->add_session(MODULE_PAYMENT_BRAINTREE_TEXT_REFUND_INITIATED, $refundAmt, $transactionid, 'success');
				return true;
